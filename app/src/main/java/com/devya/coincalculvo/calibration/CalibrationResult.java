...
package com.devya.coincalculvo.calibration;
...
public abstract class CalibrationResult {
    private static final String TAG = "Matrixx";
    ...
    public static ArrayList<Mat> tryLoad2(Activity activity, Mat cameraMatrix, Mat distortionCoefficients) {
        SharedPreferences sharedPref = activity.getPreferences(Context.MODE_PRIVATE);
        if (sharedPref.getFloat("0", -1) == -1) {
            Log.i(TAG, "No previous calibration results found");
            ...
            result.add(cameraMatrix);
            result.add(distortionCoefficients);
            return result;
        }
